...
def main():
    ...
    if save_only:
        raise NotImplementedError('The config file only support prediction, training stage is not implemented now')
    ...
    with fluid.program_guard(train_prog, startup_prog):
        with fluid.unique_name.guard():
            ...
            if FLAGS.fp16:
                """ --fp16: 是否使用混合精度训练模式 """
                assert (getattr(model.backbone, 'norm_type', None) != 'affine_channel'), \
                    '--fp16 currently does not support affine channel, please modify backbone settings to use batch norm'
            ...
            if 'use_ema' in cfg and cfg['use_ema']:
                ...
                ema = ExponentialMovingAverage(cfg['ema_decay'], thres_steps=global_steps)
                ema.update()
    ...
    if FLAGS.dist:
        dist_utils.prepare_for_multi_process(exe, build_strategy, startup_prog, train_prog)
    ...
    ignore_params = cfg.finetune_exclude_pretrained_params if 'finetune_exclude_pretrained_params' in cfg else []
    ...
    train_reader = create_reader(cfg.TrainReader, (cfg.max_iters - start_iter) * devices_num,
                                 cfg,
    ...
    for it in range(start_iter, cfg.max_iters):
        ...
        if it % cfg.log_iter == 0 and (not FLAGS.dist or trainer_id == 0):
            strs = 'iter: {}, lr: {:.6f}, {}, time: {:.3f}, eta: {}'.format(it, np.mean(outs[-1]), logs, time_cost, eta)
            logger.info(strs)
        ...
        if (it > 0 and it % cfg.snapshot_iter == 0 or it == cfg.max_iters - 1) and (not FLAGS.dist or trainer_id == 0):
            ...
            if FLAGS.eval:
                ...
                logger.info("Best test box ap: {}, in iter: {}".format(best_box_ap_list[0], best_box_ap_list[1]))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--loss_scale",     # 设置混合精度训练模式中损失值的缩放比例
        default=8.,
        type=float,
    ...
    parser.add_argument(
        "--output_eval",    # 编辑评测保存 json 的路径
        default=None,
        type=str,
        help="Evaluation directory, default is current directory.")
    parser.add_argument(
        "--use_vdl",        # 是否使用 VisualDL 记录数据，进而在 VisualDL 面板中显式
        type=bool,
        default=False,
        help="whether to record the data to VisualDL.")
    parser.add_argument(
        '--vdl_log_dir',    # 指定 VisualDL 记录数据的存储路径
        type=str,
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="If set True, enable continuous evaluation job. This flag is only used for internal test.")
